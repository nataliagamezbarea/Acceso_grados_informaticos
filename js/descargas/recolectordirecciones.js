...
async function recogerUrlsMaterial(opts) { const o = opts ||  { }
  ;
  const r = o.rama || (window.Estado ? window.Estado.obtener("rama") : "") || (window.RamaActual ? window.RamaActual.obtener() : "") || "";
  const urls = [];
  const esAdmin = Boolean(window.Permisos && window.Permisos.esAdmin);
  const filtradoInvitado = !esAdmin || Boolean(window.Permisos && window.Permisos.vistaInvitado);
    ...
    try {
      ...
      filas.forEach((f) =>  {
        ...
        aListaUrlsLocal(f.ARCHIVO).forEach((u) =>  {
          ...
          : nombre;
          urls.push( { url: u, nombre: nombreLimpio, carpeta: nombreAsigZip, rama: r }
  );
        }
  );
      }
...
function montarBotonDescargaMasiva(opts) { const o = opts ||  { }
  ;
  ...
  btn.addEventListener("click", async () =>  {
    ...
    try {
      ...
      await window.descargarTodosArchivos(urls, (estado) =>  { btn.textContent = estado; }
      ,  { nombreZip: o.nombreZip }
  );
    } catch (e) {
      ...
      setTimeout(() =>  { btn.innerHTML = `<i class="fa-solid fa-download"></i> ${o.texto}`; }
      , 2500);
    }
  }
  );
