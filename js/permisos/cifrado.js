...
const claveRam = "g1n_" + Math.random().toString(36).substring(2, 10);
  ...
  const enviarTokenAlServiceWorker = (token, repo) =>  {
    ...
    const enviar = (sw) =>  {
      if (sw) {
        try {
          sw.postMessage( { type: "SET_TOKEN", token, repo: repo || "" }
  );
        } catch (e) {
        }
      }
    ...
    try { navigator.serviceWorker.ready.then((reg) => enviar(reg.active)).catch(() =>  { }
  );
    } catch (e) {
    }
  }
  ...
  const asegurarConfigSegura = (tokenEncontrado, repoEncontrado) =>  {
    ...
    window.GITHUB_CONFIG.obtenerTokenSeguro = function () { return descifrar(this._t, this._k); }
    ;
